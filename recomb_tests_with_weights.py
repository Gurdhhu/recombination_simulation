...
- Run it with two (three?) phylogroups from Avachinskiy volcano, and each phylogroup separately
- Run it with two phylogroups from Khibine Mts, and each phylogroup separately
"""
...
logfile = f"./output_unequal/log_{timestamp}.txt"
print(f"Log will be written to {logfile}")
...
# Dictionaries of weights for determining which SSU and COI variants will be preferentially preserved
weights_ssu = {}
weights_coi = {}
...
for i in range(gener_num):
    new_gen = [combfun.combine_unequal(combfun.rand_ind(individuals_list), combfun.rand_ind(individuals_list),
                                       weights_ssu, weights_coi)
...
log_record = f"The average number of combinations per generation of the size {len(individuals_list)} is " \
             ...
             f"1 stdev: from {np.mean(comb_counts) - statistics.stdev(comb_counts)} to " \
             f"{np.mean(comb_counts) + statistics.stdev(comb_counts)}, " \
             f"2 stdev: from {np.mean(comb_counts) - 2 * statistics.stdev(comb_counts)} to " \
             f"{np.mean(comb_counts) + 2 * statistics.stdev(comb_counts)}, " \
             f"3 stdev: from {np.mean(comb_counts) - 3 * statistics.stdev(comb_counts)} to " \
             f"{np.mean(comb_counts) + 3 * statistics.stdev(comb_counts)}, " \
...
for i, gen in enumerate(gen_list[:5]):
    ...
    with open(f"./output_unequal/recombination_generation_{i+1}_{timestamp}.txt", "w") as out:
        out.write("\n".join(recomb_out))
...
#     for i in range(gener_num):
#         new_gen = [combfun.combine_unequal(combfun.rand_ind(individuals_list), combfun.rand_ind(individuals_list),
#         weights_ssu, weights_coi) for j in range(k)]
#         gen_list.append(new_gen)
#
#     subcomb_counts = []
#     comb_counts = []
...
"""
Write the counts of unique genotypes over the simulated generations into log file (can be used to make plots)
"""

log_record = "Writing the counts of unique genotypes over the simulated generations:"
combfun.write_log(logfile, log_record)

log_record = "\n".join([str(i) for i in comb_counts])
combfun.write_log(logfile, log_record)
